	...
	?>
		...
		elseif(isset($_GET['act']))
		{
			...
			elseif($_GET['act'] == 'viewal')
			{
				...
				$data = $wpdb->get_results('SELECT ALB_NAME, ALB_VIDCOUNT FROM ' . $wpdb->prefix . 'utubevideo_album WHERE ALB_ID = "' . $id . '"', ARRAY_A);
			
			?>

				<div class="utFormBox utTopformBox">
					<p class="submit utActionBar">
						<a href="<?php echo (isset($_GET['prev']) ? $_GET['prev'] : $_POST['prev']); ?>" class="utCancel utLoneNav">Go Back</a>
					</p>
					<h3>Videos for<span class="utSubH3"> ( <?php echo $data[0]['ALB_NAME']; ?> ) - <?php echo $data[0]['ALB_VIDCOUNT']; ?> videos</span></h3>
					<table class="widefat fixed utTable">
						<thead>
							<tr>
								<th>Thumbnail</th>
